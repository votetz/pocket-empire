    ...
    public ConsoleLogger() {
        ...
        bus.subscribe(event -> {
            switch (event) {
                ...
                case GameEvent.WarDeclared(var aggressor, var target, var reason) ->
                    System.out.printf("\u2694\uFE0F %s declared war on %s! (Reason: %s)%n", aggressor.getName(), target.getName(), reason);
